...
static void log_packet(const AVFormatContext *fmt_ctx, const AVPacket *pkt) {
  ...
  /*printf("pts:%s pts_time:%s dts:%s dts_time:%s duration:%s duration_time:%s
     stream_index:%d\n", av_ts_make_string(pkt->pts),
     av_ts_make_string(pkt->pts, time_base), av_ts_make_string(pkt->dts),
     av_ts_make_string(pkt->dts, time_base), av_ts_make_string(pkt->duration),
     av_ts_make_string(pkt->duration, time_base), pkt->stream_index);*/
}
...
static int write_frame(AVFormatContext *fmt_ctx, const AVRational *time_base,
  ...
  pkt->pts =
      av_rescale_q_rnd(pkt->pts, *time_base, st->time_base,
                       AVRounding(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
  pkt->dts =
      av_rescale_q_rnd(pkt->dts, *time_base, st->time_base,
                       AVRounding(AV_ROUND_NEAR_INF | AV_ROUND_PASS_MINMAX));
...
static void open_audio(AVFormatContext *oc, AVCodec *codec, AVStream *st) {
  ...
  ret = av_samples_alloc_array_and_samples(
      &src_samples_data, &src_samples_linesize, c->channels, src_nb_samples,
      AV_SAMPLE_FMT_S16, 0);
  if (ret < 0) {
  ...
  if (c->sample_fmt != AV_SAMPLE_FMT_S16) {
    ...
    ret = av_samples_alloc_array_and_samples(
        &dst_samples_data, &dst_samples_linesize, c->channels,
  ...
  dst_samples_size = av_samples_get_buffer_size(
      NULL, c->channels, max_dst_nb_samples, c->sample_fmt, 0);
}
  ...
  if (!flush) {
    ...
    if (swr_ctx) {
      ...
      if (dst_nb_samples > max_dst_nb_samples) {
        ...
        dst_samples_size = av_samples_get_buffer_size(
            NULL, c->channels, dst_nb_samples, c->sample_fmt, 0);
      }
...
static void open_video(AVFormatContext *oc, AVCodec *codec, AVStream *st) {
  ...
  if (c->pix_fmt != AV_PIX_FMT_YUV420P) {
    ret =
        avpicture_alloc(&src_picture, AV_PIX_FMT_YUV420P, c->width, c->height);
    if (ret < 0) {
...
static void fill_yuv_image(AVPicture *pict, int frame_index, int width,
                           int height) {
  int x, y, i;
  ...
  if (!flush) {
    if (c->pix_fmt != AV_PIX_FMT_YUV420P) {
      ...
      if (!sws_ctx) {
        sws_ctx =
            sws_getContext(c->width, c->height, AV_PIX_FMT_YUV420P, c->width,
                           c->height, c->pix_fmt, sws_flags, NULL, NULL, NULL);
        if (!sws_ctx) {
      ...
      sws_scale(sws_ctx, (const uint8_t *const *)src_picture.data,
                src_picture.linesize, 0, c->height, dst_picture.data,
...
int main(int argc, char **argv) {
  ...
  if (!(fmt->flags & AVFMT_NOFILE)) {
    ...
    if (ret < 0) {
      fprintf(stderr, "Could not open outfile '%s': %s", filename,
  ...
  while ((video_st && !video_is_eof) || (audio_st && !audio_is_eof)) {
    ...
    audio_time = (audio_st && !audio_is_eof)
                     ? audio_st->cur_dts * av_q2d(audio_st->time_base)
                     : INFINITY;
    video_time = (video_st && !video_is_eof)
                     ? video_st->cur_dts * av_q2d(video_st->time_base)
                     : INFINITY;

    if (!flush && (!audio_st || audio_time >= STREAM_DURATION) &&
