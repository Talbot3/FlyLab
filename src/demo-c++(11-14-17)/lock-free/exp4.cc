...
template <typename _Ty> struct NodeT {
    ...
    while (locked_flag_.test_and_set()) {
      if (_SleepWhenAcquireFailedInMicroSeconds == size_t(-1)) {
        ...
        std::this_thread::sleep_for(
            std::chrono::microseconds(_SleepWhenAcquireFailedInMicroSeconds));
      }
    }
  }
  void unlock() { locked_flag_.clear(); }
};
template <size_t _SleepWhenAcquireFailedInMicroSeconds = size_t(-1)>
...
public:
  ...
  void unlock() { locked_flag_.store(false); }
};
template <typename _Ty, typename _Lock> class LockedStackT {
...
public:
  ...
  void Push(const _Ty &val) {
    Node *node(
        new Node(val, nullptr)); // 不需要锁构造函数，这个可能是一个耗时操作
    std::lock_guard<_Lock> lock(lock_);
    node->next = head_;
    head_ = node;
  }
...
template <typename _Node> class MemoryReclamationByReferenceCountingT {
...
public:
  ...
  bool Store(_Node *node) {
    struct HazardPointerOwner {
      HazardPointer *hp;
      HazardPointerOwner(HazardPointer *hps) : hp(nullptr) {
        for (size_t i = 0; i < _MaxPopThreadCount; ++i) {
          std::thread::id id;
          if (hps[i].id.compare_exchange_strong(id,
...
template <typename _Ty, typename _MemoryReclamation> class LockFreeStackT {
  typedef NodeT<_Ty> Node;
...
struct LockFreePerformanceTestT {
  template <class _ProcessUnit> static double Run(_ProcessUnit puf) {
...
int main() {
  ...
  std::cout << "LockFreeStack with MemoryReclamationByReferenceCounting"
            << "\t\t";
  LockFreePerformanceTestT<LockFreeStackT<
      uint32_t, MemoryReclamationByReferenceCountingT<NodeT<uint32_t>>>>::Run();
  std::cout << "LockFreeStack with MemoryReclamationByHazardPointer"
            << "\t\t";
  LockFreePerformanceTestT<LockFreeStackT<
      uint32_t, MemoryReclamationByHazardPointerT<NodeT<uint32_t>>>>::Run();
  return 0;
}