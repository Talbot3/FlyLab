...
ip_haikang = '34020000001320000001'         # 海康端口IP信息18062904141310236235
device_dict = {
               '34020000001320000001@192.168.28.110:5060': [5060, 192, 168, 28, 110],
...
def get_messages_receive(s, addr_get):
    ...
    str_send += list1
    with open('RequestDevice.txt', 'a') as f:
        f.write(str_send+'\r\n')
    b4 = str_send.encode()
...
def get_video_receive(s, addr_get):
    ...
    str_send += 'From: <sip:0000042001000001@{}:{}>;tag={}\n'.format(local_ip, local_port, 231423)
    str_send += 'To: <sip:{}{}>\n'.format(i, ip_haikang[ip_haikang.find('@'):])
    str_send += 'Call-ID: {}\n'.format(30013)
    ...
    str_send += 'Content-Length: {}\n\n'.format(len(list1))
    str_send += list1
    with open('RequestStream.txt', 'a') as f:
        f.write(str_send+'\r\n')
    b4 = str_send.encode()
    s.sendto(b4, addr_get)
    port += 2
...
def sb_dat(s, addr_interface):
    ...
    device_dict_del = []
    dizhi = '/Users/Arthur/FlyLab/src/demo-GB28181/video/'
