 ...
 * @extends {Stream}
 */
var AccStream = function AccStream(name, batchCount, timeout) {
	...
	if(this._timeout > 0 ) {
		this._currentTimestamp = new Date().getTime();
	}
...
AccStream.prototype._middleWareSync = function(data) {
	...
	} else if (this._timeout > 0 && this._currentTimestamp + this._timeout > new Date().getTime()) {
		return this.deQueueBatch();
	}
};
